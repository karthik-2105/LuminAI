...
class LuminAI:
    ...
    def get_vectorstore(self, text_chunks: List[str]):
        try:
            # Use HuggingFaceEmbeddings with the specified model name
    ...
    def get_conversation_chain(self, vectorstore):
        try:
            ...
            # Memory with explicit output key
            ...
            return ConversationalRetrievalChain.from_llm(
                ...
                output_key="answer"  # Add output key here as well
            )
    ...
    def get_conversation_chain(self, vectorstore):
        try:
            ...
            custom_template = """Use the following conversation history and context to answer the question. 
            ...
            # Load model and tokenizer with better settings
            tokenizer = AutoTokenizer.from_pretrained("gpt2")
            model = AutoModelForCausalLM.from_pretrained("gpt2", device_map="auto")
            ...
            # Configure memory to properly maintain conversation history
            memory = ConversationBufferMemory(
                memory_key="chat_history",
                return_messages=True,
                output_key="answer",
            ...
            # Create chain with proper configuration
            chain = ConversationalRetrievalChain.from_llm(
                llm=llm,
                retriever=vectorstore.as_retriever(search_kwargs={"k": 3}),  # Retrieve more context
                memory=memory,
                return_source_documents=True,
    ...
    def handle_userinput(self, user_question: str):
        try:
            ...
            response = st.session_state.conversation({
                'question': user_question,
                'chat_history': st.session_state.chat_history if st.session_state.chat_history else []
            })
            ...
            for i, message in enumerate(st.session_state.chat_history):
                ...
                if st.session_state.use_typewriter and i == len(st.session_state.chat_history) - 1:
                    self.typewriter.write(message.content, template)
                else:
            ...
            Sources:
            {chr(10).join(s.page_content for s in response['source_documents'])}
    ...
    def main(self):
        st.title("LuminAI2 Research Assistant")
